...
describe('relative json pointer util', function () {
  ...
  describe('resolve to value', function () {
    it('resolves 0', function () {
      expect(resolve(data, '/foo/1', '0')).to.eql('baz');
    });

    it('resolves 1', function () {
      expect(resolve(data, '/foo/1', '1')).to.eql(['bar', 'baz']);
    });

    it('resolves 1/0', function () {
      expect(resolve(data, '/foo/1', '1/0')).to.eql('bar');
    });

    it('resolves 1/highly/nested/objects', function () {
      expect(resolve(data, '/foo/1', '2/highly/nested/objects')).to.eql(true);
    });

    it('does not throw if 0 value and root reference', function () {
      expect(resolve(data, '/', '0')).to.eql(data);
    });
    ...
    it('resolves 0/objects', function () {
      expect(resolve(data, '/highly/nested', '0/objects')).to.eql(true);
    });
  });

  describe('resolve to property name', function () {
    it('resolves to index in array', function () {
      expect(resolve(data, '/foo/1', '0#')).to.eql(1);
    });

    it('resolves to property name', function () {
      expect(resolve(data, '/foo/1', '1#')).to.eql('foo');
      expect(resolve(data, '/highly/nested', '0#')).to.eql('nested');
      expect(resolve(data, '/highly/nested', '1#')).to.eql('highly');
    });
  });
});
